...
const AnimatedLoadingSkeleton = () => {
    ...
    const generateSearchPath = (config: GridConfig) => {
        ...
        if (allPositions.length > 0) {
            shuffledPositions = allPositions
        ...
        return {
            ...
            transition: {
                ...
                times: shuffledPositions.length > 1 ? shuffledPositions.map((_, i) => i / (shuffledPositions.length - 1)) : [1]
            }
        }
    }
    ...
    const config = getGridConfig(windowWidth || 1024) // Get current grid configuration with default
